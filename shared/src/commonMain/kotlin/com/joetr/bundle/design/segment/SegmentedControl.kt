package com.joetr.bundle.design.segment
...
private val TRACK_COLOR: Color
    @Composable
    get() {
        return MaterialTheme.colorScheme.onSurface.copy(alpha = 0.2f)
    }
...
@Composable fun SegmentText(text: String) {
    Text(text, maxLines = 1, overflow = Ellipsis, color = MaterialTheme.colorScheme.onSurface)
}
...
@Composable private fun Thumb(state: SegmentedControlState) {
    Box(
        Modifier
            ...
            .background(MaterialTheme.colorScheme.surface, BACKGROUND_SHAPE),
    )
}
